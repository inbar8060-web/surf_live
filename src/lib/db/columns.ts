...
// prettier-ignore
export const CLIENT_COLUMNS = 'profile_id, level, birth_date, emergency_contact_name, emergency_contact_phone, medical_notes, waiver_signed_at, height_cm, weight_kg, created_at, updated_at' as const

/** A client row as a browser session may read it — no internal notes. */
export type ReadableClient = Omit<Client, 'admin_notes'>

// prettier-ignore
export const INSTRUCTOR_COLUMNS = 'profile_id, bio, specialties, certifications, languages, whatsapp_phone, calendar_color, hired_at, created_at, updated_at' as const

/** An instructor row as a browser session may read it — no payout handle. */
export type ReadableInstructor = Instructor

// prettier-ignore
export const RESERVATION_COLUMNS = 'id, slot_id, client_id, status, participants, client_package_id, price_cents, currency, payment_id, client_note, rejection_reason, revision, decided_by, decided_at, created_at, updated_at' as const
