...
import { getRequestClub, clubUrl } from '@/lib/tenant'
...
async function clubAudit(actorId: string, action: string, clubId: string, detail?: Record<string, unknown>) {
...
export async function startPlanCheckoutAction(
  ...
  const [settings, club] = await Promise.all([getClubSettings(), getRequestClub()])
  const base = club ? clubUrl(club.slug, '') : ''
  ...
  await clubAudit(admin.id, 'subscription.checkout_started', clubId, { plan: plan.key })
  redirect(url)
}
...
export async function changePlanAction(
  ...
  await clubAudit(admin.id, 'subscription.plan_changed', clubId, { from: current.plan_key, to: plan.key })
...
export async function cancelPlanAction(_prev: ActionResult<null> | null, formData: FormData): Promise<ActionResult<null>> {
  ...
  await createAdminClient()
    ...
    .eq('id', current.id)
  await clubAudit(admin.id, 'subscription.canceled_by_club', clubId, { plan: current.plan_key })
  revalidatePath('/admin', 'layout')
  return ok(null, 'The plan is cancelled. The desk stays open until the end of the paid period.')
}
...
export async function startPayoutOnboardingAction(): Promise<ActionResult<null>> {
  ...
  await clubAudit(admin.id, 'payouts.onboarding_started', clubId)
  redirect(result.url)
}
...
export async function setPlatformFeeAction(_prev: ActionResult<null> | null, formData: FormData): Promise<ActionResult<null>> {
  ...
  await clubAudit(operator.id, 'payouts.fee_set', parsed.data.clubId, { fee_bps: bps })
